...
fabric_ns = {'fabric': 'http://schemas.microsoft.com/2011/01/fabric'}
...
def find_service_manifest(xml_file):
    ...
    import_elem = root.find('fabric:ServiceManifestImport', fabric_ns)
    if import_elem is None:
        raise ValueError('Could not find service manifest import section')

    ref_elem = import_elem.find('fabric:ServiceManifestRef', fabric_ns)
    if ref_elem is None:
...
def parse_service_type(xml_file):
    ...
    type name"""
    root = ET.parse(xml_file).getroot()
    service_type = root.find('fabric:ServiceTypes', fabric_ns)
